...
void sistema::evo(uint32_t mode, int st){
	...
	*/
	...
	for(uint64_t i=0; i<n; i++){
		...
		struct timespec ts = { 0, 1 }; // Seconds, nanoseconds
...
void sistema::evodt(uint32_t mode, uint32_t j){
	/*std::vector<corpo*>::iterator it;	
	for(it=m_corpi.begin(); it!=m_corpi.end(); it++){
		*it->evolvidT(m_corpi, m_dT);
	}*/
	for(int i=0; i<m_corpi.size(); i++){
		m_corpi[i]->evolvidT(m_corpi, m_dT, mode, j);
	}
	/*
	//metto if che vale solo per una simulazione perché voglio farla così

	vettore dS=m_corpi[0]->P();
	std::ofstream out;
	for(auto c: m_corpi){
		out.open(m_inc+c->NAME()+"_dist_sole.txt", std::ofstream::app);
		vettore dd=c->P()-dS;
...
//	out<<std::endl;*/
	...
	double Emec=0;
  	...
  	//raccgliere dati precessione - casino perché devo campionare in modo diverso per ogni corpo, oppure lo faccio solo per Mer
...
void sistema::clean(){
	/*
	for(auto c: m_corpi){
		std::string file=m_inc+c->NAME()+"_dist_sole.txt";
		std::ofstream out(file);
//		out<<c->NAME()<<": distanza dal sole in fz del tempo (step)"<<std::endl;
		out.close();
	}*/
